...
# df_construction.to_csv('run3.csv', index=False)  
# print("DB construction is written to a csv file!")

# ##TODO: check this code reading form the file and loading it to DB 
# ## since it is not working correctly. It is independent from inserting and searching
# ## Read DB from a csv file
# db.reset_stats()
# _constructedDB = pd.read_csv('db10k64m32c.csv')
# # print(len(_constructedDB.index))
# _constructedDB_stats = db.get_stats()
# ## DB loading
# constructedDB = []
# for i in range(int(len(_constructedDB))):
#     _tpl_construct = _constructedDB.iloc[i].Template
#     _id_construct = db.insert(_tpl_construct)
#     constructedDB.append((_id_construct, _tpl_construct))
# constructedDB_stats = db.get_stats()
# past_stats().append(constructedDB_stats)
# df_constructedDB = pd.DataFrame(constructedDB, columns=["ID", "Template"])
# pd.DataFrame(past_stats())
# print(pd.DataFrame(past_stats()))
# print("DB is read from a csv file!")
